<div class="blog">
		...
		<div class="blogimage">
			<div class="text">
				<h1><?php echo $model->article_name; ?>
				<?php 
					/*if(isset($model->article_name) && $model->article_name!=''){
						$article_name = substr($model->article_name,0,40);
						if(strlen($model->description)>40){
							echo $article_name.'...';
						}
						else
						{
...
Chances are that if you’ve lived more than a couple decades, you’ve experienced this in some form or another. Getting a promotion at your job doesn’t necessarily make you more confident in your professional abilities. In fact, it can often make you feel less confident. Dating and/or sleeping with more people doesn’t necessarily make you feel more confident about how attractive you are. Moving in together or getting married doesn’t necessarily make you feel any more confident in your relationship.</li>
		...
		<div class="section grid related-post">
			...
			<div class="item_container">
				...
				{
					$model = Article::model()->findAll(array('select'=>'*, rand() as rand',"condition"=>'id !='.$model->id,'order'=>'rand','limit'=>'5'));
				}
				else
				{
					$model = Article::model()->findAll(array('select'=>'*, rand() as rand','condition'=>'is_premium = 0 and id !='.$model->id,'order'=>'rand','limit'=>'5'));
				}
					...
					<div class="item">
						...
						<div class="text" style="height: 200px !important;overflow: hidden;">
							<h4>
								...
								?>
								<p><?php echo $date = date('M d, Y', strtotime($data->created_at)) ?></p>
							</h4>
							...
							<a href="<?php echo Yii::app()->createUrl('site/article_details',array('id'=>$data->id));?>">
								<p class="graytext">
								<?php $string = substr($data->description,0,200);
