...
class Bookmark extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('user_id, article_id', 'required'),
			array('user_id, article_id', 'numerical', 'integerOnly'=>true),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, user_id, article_id', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
			'article_table'=>array(self::BELONGS_TO, 'Article', 'article_id'),
		);
	}
	...
	public function search()
	{
		...
		$criteria->with=array('article_table');
		$id=Yii::app()->user->id;
		...
		{
			$criteria->compare('article_table.is_premium',0);
		}
